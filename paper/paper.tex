...
\def\viperRTsmall{TBD}
\def\viperRTmedium{TBD}
\def\viperRTlarge{TBD}
...
\begin{table}[t]
\centering
\caption{Random tester: TU vs.\ stock GPU\_VIPER.}
\label{tab:viper-cmp}
\small
\begin{tabular}{lrr}
\toprule
\textbf{System} & \textbf{Stock VIPER} & \textbf{Spandex TU} \\
\midrule
small         & \viperRTsmall   & \spxRTsmall   \\
medium        & \viperRTmedium  & \spxRTmedium  \\
large/TL=100  & \viperRTlarge   & \spxRTlarge   \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:viper-cmp} compares Spandex TU against stock GPU\_VIPER
on the random tester. \emph{[Discussion of relative performance fills
in once VIPER baseline data lands.]}

\subsection{HIP benchmarks}

\begin{table}[t]
\centering
\caption{HIP benchmarks: square (smoke test), Pannotia BC and PageRank
(\texttt{1k\_128k.gr} input).}
\label{tab:pannotia}
\small
\begin{tabular}{lrr}
\toprule
\textbf{Benchmark} & \textbf{Stock VIPER} & \textbf{Spandex TU} \\
\midrule
square   & \sqViperTicks & \sqSpxTicks \\
BC       & \bcViperTicks & \bcSpxTicks \\
PageRank & \prViperTicks & \prSpxTicks \\
\bottomrule
\end{tabular}
\end{table}

We use \texttt{square} as a minimal smoke test (a self-contained
square-the-numbers kernel exercising basic GPU load/store/atomic
paths) plus the two Pannotia graph applications, which stress
fine-grained atomics on shared vertex properties.
